...
module.exports = grammar({
  ...
  rules: {
    ...
    html_template: ($) =>
      seq(
        field("start", alias("<", "html_template_start")),
        field("tag", $.identifier),
        field("attributes", optional($.html_template_attributes)),
        choice(
          seq(
            field("end", alias(">", "html_template_end")),
            optional(
              $.html_children,
            ),
            field("start", alias("</", "html_template_start")),
    ...
    html_template_attributes: ($) =>
      repeat1(
        seq(
          field("name", $.identifier),
          "=",
          field("value", $.html_template_attribute_value)
        )
      ),

    html_template_attribute_value: ($) => choice($.string, seq("{", $._expression, "}")),
    ...
    html_children: ($) =>
      repeat1(
        choice(
          $.html_template,
          $.html_template_text,
          $.html_template_expression,
        )
      ),

    html_template_text: ($) => /[^<{]+/,
    html_template_expression: ($) => seq("{$", $._expression, "$}"),
